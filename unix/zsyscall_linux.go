...
func Listns(req *NsIdReq, nsIds []uint64, flags uint) (n int, err error) {
	var _p0 unsafe.Pointer
	if len(nsIds) > 0 {
		_p0 = unsafe.Pointer(&nsIds[0])
	} else {
		_p0 = unsafe.Pointer(&_zero)
	}
	r0, _, e1 := Syscall6(SYS_LISTNS, uintptr(unsafe.Pointer(req)), uintptr(_p0), uintptr(len(nsIds)), uintptr(flags), 0, 0)
	n = int(r0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
